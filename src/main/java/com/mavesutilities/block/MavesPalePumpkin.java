...
public class MavesPalePumpkin extends PumpkinBlock {

    public MavesPalePumpkin(AbstractBlock.Settings settings) {
        super(settings);
    }
    ...
    protected ActionResult onUseWithItem(ItemStack stack, BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (!stack.isOf(Items.SHEARS)) {
            ...
            world.setBlockState(pos, MavesBlocks.CARVED_PALE_PUMPKIN.getDefaultState().with(MavesCarvedPalePumpkin.FACING, direction2), Block.NOTIFY_ALL_AND_REDRAW);
            ItemEntity itemEntity = new ItemEntity(
                    world,
                    (double)pos.getX() + 0.5 + (double)direction2.getOffsetX() * 0.65,
                    (double)pos.getY() + 0.1,
                    (double)pos.getZ() + 0.5 + (double)direction2.getOffsetZ() * 0.65,
                    new ItemStack(MavesItems.PALE_PUMPKIN_SEEDS, 4)
            );
