...
public class MavesCarvedPalePumpkin extends CarvedPumpkinBlock {
    ...
    private static final Predicate<BlockState> IS_GOLEM_HEAD_PREDICATE = state -> state != null
            && (state.isOf(MavesBlocks.CARVED_PALE_PUMPKIN) || state.isOf(MavesBlocks.PALE_JACK_O_LANTERN));

    public MavesCarvedPalePumpkin(Settings settings) {
        super(settings);
